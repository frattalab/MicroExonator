...
This allows us to visualise microexon coverages in IGV with standard reference genomes, gene models etc. *A lot simpler* (for me) than reformatting BAM reference names and cigar strings (though would be great to be able to visualise reads/read alignments!). Preprint has some Sashimi plots (and scripts in src/ to do this) but this is not automated (and cannot interact with plots?)...

##### Generate output results table with 'annotation status' (is microexon annotated or novel?)
Existing script `src/stats/discovery_stats.py` is not included in workflow and only seems to take BED12 annotation to check whether microexon has been previously identified. I wrote a quick and dirty script `src/classify_me_annotation_status.py` that generates tab-separated `Report/out*.annot_status.txt` files. These have an appended `annotation_status` column with values of `annotated` or `novel` dependent on whether found in provided annotation files (BED12 + VastDB if provided).

##### Output data to a 'master results directory'
See *configuration* section - specify master output directory with `config["master_output_directory"]` in *config.yaml* - all output from your run will be stored here. I can't see any advantages to the previous approach (would have to copy output elsewhere before submitting a run, or reclone repo elsewhere (and re-install all conda envs!))
...
##### On the list - Simplify handling local FASTQ files
Copying seems a bit wasteful - consider soft-linking to original files (then unzipping locally to run)

Also other random updates to README and little typo fixes in scripts that I've noticed and reported. There is now a readthedocs hosted on main repo with more details on running  MicroExonator than here...
...
Before running MicroExonator you need to have certain files in the `MicroExonator/` directory. By default, all the downloaded files and the results will be stored in the `MicroExonator/` folder which means that several GBs of disk space may be required. First, you need to create a `config.yaml`, which should contain the path of the input files and certain paramethers. The format for the `config.yaml` file is shown below:
    ...
    #make sure master_output_directory has a trailing slash
    master_output_directory : master_outdir/
...
* `master_output_directory` is the path to the directory in which you want to store all output from this MicroExonator run. It **must have a trailing slash** or file-paths will not be constructed correctly.
...
Exit interactive julia session (`control + d`) and find Whippet's binary folder, that should be inside of your miniconda environment folder. Once you find the path to this folder, add it to `config.yaml` writing the following lines:
    ...
    #Would recommend generating Gene_anontation_bed12 from this GTF file (UCSC Tools...) so they match up
